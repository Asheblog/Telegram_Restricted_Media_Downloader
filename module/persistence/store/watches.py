...
class WatchesMixin:
    ...
    def upsert_live_transfer_watch(
        ...
        delay_value = (
            None if comment_delay_minutes is None else int(comment_delay_minutes)
        )
    ...
    def get_live_transfer_watch(self, watch_id: str) -> Optional[Dict[str, Any]]:
        with self.connect() as conn:
            row = conn.execute(
                "SELECT * FROM live_transfer_watches WHERE id = ?", (watch_id,)
            ).fetchone()
    ...
    def delete_live_transfer_watch(self, watch_id: str) -> bool:
        with self.connect() as conn:
            cursor = conn.execute(
                "DELETE FROM live_transfer_watches WHERE id = ?", (watch_id,)
            )
            return cursor.rowcount > 0
    ...
    def list_live_watch_events(
        ...
        with self.connect() as conn:
            ...
            total = int(
                conn.execute(
                    f"SELECT COUNT(*) FROM live_watch_events WHERE {where_sql}", params
                ).fetchone()[0]
            )
            rows = conn.execute(
                f"""
    ...
    def delete_live_watch_events(self, watch_id: str) -> int:
        with self.connect() as conn:
            cursor = conn.execute(
                "DELETE FROM live_watch_events WHERE watch_id = ?", (watch_id,)
            )
