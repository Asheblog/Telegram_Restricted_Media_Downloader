...
def _cancel_row() -> list[InlineKeyboardButton]:
    return [InlineKeyboardButton(BotButton.CANCEL, callback_data=BotCallbackText.GUIDE_WIZARD_CANCEL)]
...
class BotGuideWizard:
    ...
    async def handle_callback(
        ...
        if session is None:
            await callback_query.message.edit_text('⚠️引导任务已结束或超时，请重新发送命令。')
            return True
    ...
    def _build_prompt(self, session: GuideWizardSession) -> tuple[str, InlineKeyboardMarkup]:
        if session.step == WizardStep.SOURCE_CHANNEL:
        ...
        if session.step == WizardStep.RANGE_DETECTED:
            return (
                ...
                '确认使用该范围，或自行指定起止消息链接。',
                _markup(
                    [InlineKeyboardButton('✅ 确认范围', callback_data=BotCallbackText.GUIDE_WIZARD_CONFIRM_RANGE)],
                    [InlineKeyboardButton('✏️ 自己指定', callback_data=BotCallbackText.GUIDE_WIZARD_CUSTOM_RANGE)],
                ),
            )
        if session.step == WizardStep.START_MESSAGE_LINK:
            return (
        ...
        if session.step == WizardStep.INCLUDE_COMMENT:
            return (
                ...
                _markup(
                    [InlineKeyboardButton('✅ 包含评论区', callback_data=BotCallbackText.GUIDE_WIZARD_COMMENT_YES)],
                    [InlineKeyboardButton('❌ 不含评论区', callback_data=BotCallbackText.GUIDE_WIZARD_COMMENT_NO)],
                ),
            )
        if session.step == WizardStep.LISTEN_CHANNEL:
            return (
        ...
        if session.step == WizardStep.LISTEN_COLLECT:
            ...
            return (
                ...
                '可继续添加，或完成并开始监听。',
                _markup(
                    [InlineKeyboardButton('➕ 再添加一个', callback_data=BotCallbackText.GUIDE_WIZARD_ADD_LISTEN)],
                    [InlineKeyboardButton('✅ 完成并开始监听', callback_data=BotCallbackText.GUIDE_WIZARD_FINISH_LISTEN)],
                ),
            )
        return self._confirm_prompt(session), _markup(
            [InlineKeyboardButton('▶️ 确认执行', callback_data=BotCallbackText.GUIDE_WIZARD_CONFIRM_EXECUTE)],
        )
