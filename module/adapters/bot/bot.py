...
class Bot:
    ...
    async def help(
        ...
        text = (
            ...
            f'上游作者: Gentlesprite | 魔改作者: Asheblog\n'
            f'©️ {__copyright__.replace(" <https://github.com/Gentlesprite>", ".")} | {__license__}`\n\n'
            f'⭐ 本 Fork 亮点:\n'
            f'• **深链取片**: 频道帖只有资源 bot 按钮时,自动取正片再转存(含评论区深链)。\n'
            f'• **WebUI 任务中心**: 转存/监听、进度与失败原因、暂停继续、监听历史与导出。\n'
            f'• **PikPak 归档**: rclone 按来源频道与帖子路径整理,深链内容仍归属原帖。\n\n'
            f'🌐 WebUI 入口:\n'
            f'长期任务推荐用 WebUI 管理。\n'
            f'Docker 默认访问: `http://服务器IP:2921`\n'
            f'自定义端口以日志 `WebUI已启动: ...` 为准。\n'
