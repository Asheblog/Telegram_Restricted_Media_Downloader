...
class UserConfig(BaseConfig):
    ...
    def __init__(self):
        ...
        self.max_download_task: int = (self.config.get("max_tasks") or {}).get(
            "download", 1
        )
        self.max_download_retries: int = (self.config.get("max_retries") or {}).get(
            "download", 5
        )
        self.max_upload_task: int = (self.config.get("max_tasks") or {}).get(
            "upload", 1
        )
        self.max_upload_retries: int = (self.config.get("max_retries") or {}).get(
            "upload", 3
        )
        ...
        self.temp_directory: str = PARSE_ARGS.temp or (
            self.config.get("temp_directory") or UserConfig.TEMP_DIRECTORY
        )
    ...
    def config_guide(self) -> None:
        ...
        try:
                ...
                if re_config:
                    ...
                    if not PARSE_ARGS.session:
                        ...
                        if self.is_change_account:
                            pre_load_config["session_directory"] = (
            ...
            _max_download_task: Union[int, None] = pre_load_config.get(
                "max_tasks", {"download": 1}
            ).get("download")
            _max_download_retries: Union[int, None] = pre_load_config.get(
            ...
            if (
                ...
                if not _api_hash:
                    api_hash, record_flag = gsp.get_api_hash(
                        last_record=self.last_record.get("api_hash"), valid_length=32
                    ).values()
                    if record_flag:
                        self.record_flag = record_flag
            ...
            if not _bot_token and self.re_config:
                ...
                if enable_bot:
                    bot_token, record_flag = gsp.get_bot_token(
                        last_record=self.last_record.get("bot_token"), valid_format=":"
                    ).values()
                    if record_flag:
                        self.record_flag = record_flag
                        pre_load_config["bot_token"] = bot_token
            if (
                not _links or not _bot_token and self.re_config
            ) and PARSE_ARGS.web is None:
            ...
            if not _save_directory or self.re_config:
                save_directory, record_flag = gsp.get_save_directory(
                    last_record=self.last_record.get("save_directory")
                ).values()
                if record_flag:
                    self.record_flag = record_flag
            ...
            if not _max_download_task or self.re_config:
                max_download_task, record_flag = gsp.get_max_download_task(
                    last_record=self.last_record.get("max_tasks", {"download": 1}).get(
                        "download"
                    )
                ).values()
                if record_flag:
            ...
            if not _download_type or self.re_config:
                download_type, record_flag = gsp.get_download_type(
                    last_record=self.last_record.get("download_type")
                ).values()
                if record_flag:
                    self.record_flag = record_flag
            ...
            if _is_shutdown is None or self.re_config:
                is_shutdown, _is_shutdown_record_flag = gsp.get_is_shutdown(
                    last_record=self.last_record.get("is_shutdown"), valid_format="y|n"
                ).values()
        ...
        if exit_flag:
            raise SystemExit(0)
        pre_load_config.get("max_tasks", {"download": _max_download_task, "upload": 1})[
            "download"
        ] = (pre_load_config.get("max_tasks") or {}).get("download", 1) or 1
        pre_load_config.get("max_tasks", {"download": 1, "upload": 1})["upload"] = (
            pre_load_config.get("max_tasks") or {}
        ).get("upload", 1) or 1
        pre_load_config.get(
            "max_retries", {"download": _max_download_retries, "upload": 3}
        )["upload"] = (pre_load_config.get("max_retries") or {}).get("upload", 3) or 3
        pre_load_config = self.normalize_runtime_numbers(pre_load_config)
        if not PARSE_ARGS.session:
            _session_directory = (
                pre_load_config.get("session_directory", UserConfig.WORK_DIRECTORY)
        ...
        if not PARSE_ARGS.temp:
            _temp_directory = (
                pre_load_config.get("temp_directory", UserConfig.TEMP_DIRECTORY)
...
class GlobalConfig(BaseConfig):
    ...
    TEMPLATE: dict = {
        ...
        "message_filter": {
            ...
            "date_range": {"enabled": False, "start_date": None, "end_date": None},
            "keywords": {"enabled": False, "words": ["广告", "推广", "赞助"]},
        },
        "live_watch": {
            "comment_delay_minutes": 20,
        },
        "transfer": {
            "item_stale_timeout_minutes": 5,
        },
        "deep_link": {
            "bot_whitelist": [],
            "timeout_seconds": 60,
    ...
    def __init__(self):
        ...
        self.default_transfer_nesting = self.TEMPLATE.get("transfer")
        ...
        self.target_profiles: dict = self.config.get(
            "target_profiles", self.default_target_profiles_nesting
        )
        self.forward_type: dict = self._resolve_forward_type()
        self.message_filter: dict = self.config.get(
            "message_filter", self.default_message_filter_nesting
        )
        self.live_watch: dict = self.config.get(
            "live_watch", self.default_live_watch_nesting
        )
        self.comment_delay_minutes: int = self.get_comment_delay_minutes()
        self.transfer: dict = self.config.get("transfer", self.default_transfer_nesting)
        self.item_stale_timeout_minutes: int = self.get_item_stale_timeout_minutes()
        self.deep_link: dict = self.config.get(
            "deep_link", self.default_deep_link_nesting
        )
        ...
        self.deep_link_min_interval_seconds: int = (
            self.get_deep_link_min_interval_seconds()
        )
        ...
        self.deep_link_page_click_interval_seconds: int = (
            self.get_deep_link_page_click_interval_seconds()
        )
    ...
    def get_local_storage_reserve_bytes(self) -> int:
        default_value = self.default_upload_nesting.get(
            "local_storage_reserve_bytes", 1073741824
        )
        try:
            value = int(
    ...
    def get_comment_delay_minutes(self) -> int:
        default_value = int(
            self.default_live_watch_nesting.get("comment_delay_minutes", 20)
        )
        try:
            value = int(
                self.get_nesting_config(
    ...
    def get_item_stale_timeout_minutes(self) -> int:
        default_value = int(
            self.default_transfer_nesting.get("item_stale_timeout_minutes", 5)
        )
        try:
            value = int(
                self.get_nesting_config(
                    default_nesting=self.default_transfer_nesting,
                    param="transfer",
                    nesting_param="item_stale_timeout_minutes",
                )
            )
        except (TypeError, ValueError, AttributeError):
            value = default_value
    ...
    def get_deep_link_min_interval_seconds(self) -> int:
        default_value = int(
            self.default_deep_link_nesting.get("min_interval_seconds", 30)
        )
        try:
            value = int(
                self.get_nesting_config(
    ...
    def get_deep_link_page_click_interval_seconds(self) -> int:
        default_value = int(
            self.default_deep_link_nesting.get("page_click_interval_seconds", 1)
        )
        try:
            value = int(
                self.get_nesting_config(
    ...
    def save_config(self, config: dict) -> None:
        ...
        self.target_profiles = self.config.get(
            "target_profiles", self.default_target_profiles_nesting
        )
        ...
        self.transfer = self.config.get("transfer", self.default_transfer_nesting)
        self.item_stale_timeout_minutes = self.get_item_stale_timeout_minutes()
        ...
        self.deep_link_page_click_interval_seconds = (
            self.get_deep_link_page_click_interval_seconds()
        )
        p = "全局配置文件已重新加载。"
    ...
    def process_target_profiles(self, config: dict) -> None:
        ...
        for profile_name, profile_template in self.TEMPLATE.get(
            "target_profiles", {}
        ).items():
    ...
    def __check_params(self, config: dict) -> None:
        ...
        self.process_nesting(param_name="transfer", config=config)
