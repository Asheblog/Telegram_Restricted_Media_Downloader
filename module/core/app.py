...
class Application(UserConfig, StatisticalTable):
    ...
    def refresh_runtime_fields(self) -> None:
        ...
        self.max_download_retries = (self.config.get("max_retries") or {}).get(
            "download", 5
        )
        self.max_upload_task = (self.config.get("max_tasks") or {}).get("upload", 1)
        self.max_upload_retries = (self.config.get("max_retries") or {}).get(
            "upload", 3
        )
        ...
        self.temp_directory = PARSE_ARGS.temp or (
    ...
    def get_temp_file_path(
        self, message: pyrogram.types.Message, dtype: str, title_override: str = None
    ) -> str:
        """获取下载文件时的临时保存路径。"""
        ...
        dt = DownloadFileName(
            message=message, download_type=dtype, title_override=title_override
        )
...
class DownloadFileName:
    ...
    def get_message_title(self) -> Union[str, None]:
        ...
        if isinstance(self.title_override, str):
            ...
            if title:
                return (
                    validate_title(title[: self.TITLE_MAX_CHARS]).strip(" ._") or None
                )
