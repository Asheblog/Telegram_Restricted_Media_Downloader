...
class TrmdCompositionRoot:
    ...
    def __init__(self):
        ...
        self.download_upload_window = DynamicAsyncWindow(
            limit_provider=lambda: self.gc.upload_pending_limit, minimum=1, maximum=5
        )
        self.local_storage_guard = LocalStorageGuard(
            reserve_bytes_provider=lambda: getattr(
                self.gc,
        ...
        self.web_task_manager = WebUITaskManager(
            ...
            web_running_task_id_setter=lambda v: setattr(
                self, "web_running_task_id", v
            ),
            ...
            set_live_watch_status_getter=lambda: getattr(
                self, "set_live_watch_status", None
            ),
            watch_payload_from_record_getter=lambda: getattr(
                self, "watch_payload_from_record", None
            ),
    ...
    def _ensure_watch_manager(self):
        ...
        self.watch_manager = LiveWatchManager(
            ...
            operation_submitter=getattr(
                ...
                lambda ot, p: {"id": f"{ot}-0", "status": TransferStatus.PENDING},
            ),
    ...
    def _ensure_pikpak_manager(self):
        ...
        self.pikpak_manager = PikpakIntegrationManager(
            ...
            refresh_counts=lambda tid: (
                ...
                if self.__dict__.get("transfer_store")
                else None
            ),
            cleanup_item_file=lambda item_id: (
                self._ensure_media_manager().try_cleanup_item_file(item_id)
            ...
            schedule_deferred_archive=lambda **kwargs: (
                ...
                and self.__dict__.get(
                    "progress_tracker"
                )._schedule_deferred_upload_archive(**kwargs)
            ),
        )
    ...
    def _ensure_progress_tracker(self):
        ...
        self.progress_tracker = TransferProgressTracker(
            ...
            cleanup_local_file=lambda item_id: getattr(
                self.__dict__.get("media_manager"),
                "try_cleanup_item_file",
                lambda i: False,
            )(item_id),
        )
