...
class LiveWatchManager:
    def __init__(
        ...
        self.listen_download_chat = (
            listen_download_chat if listen_download_chat is not None else {}
        )
        self.listen_forward_chat = (
            listen_forward_chat if listen_forward_chat is not None else {}
        )
        self.web_pending_watches = (
            web_pending_watches if web_pending_watches is not None else {}
        )
        self.web_watch_handler_clients = (
            web_watch_handler_clients if web_watch_handler_clients is not None else {}
        )
    ...
    def set_live_watch_status(
        self, watch_id: str, status: str, error_message: str = None
    ) -> None:
        ...
        if store:
            store.update_live_transfer_watch_status(
                watch_id=watch_id, status=status, error_message=error_message
            )
    ...
    def list_watches(self, tz_offset_minutes: int | None = None) -> list:
        ...
        for watch_id in watches_by_id:
            ...
            if store and watches_by_id[watch_id].get("type") == "forward":
                ...
                today_count = store.get_live_watch_event_count(
                    watch_id, today_only=True, tz_offset_minutes=tz_offset_minutes
                )
                deferred_count = len(
                    store.list_deferred_discussion_captures(
                        watch_id=watch_id,
        ...
        watches = sorted(
            watches_by_id.values(), key=lambda watch: str(watch.get("id") or "")
        )
        return watches
    ...
    def mark_pending_watch(
        self, payload: dict, status: str, error_message: str = None
    ) -> None:
    ...
    def _create_live_watch_operation(self, watch_type: str, payload: dict) -> str:
        if self._operation_submitter:
            operation = self._operation_submitter(
                "watch", {"watch_type": watch_type, **payload}
            )
            return operation["id"]
        return ""
    ...
    def create_watch(self, payload: dict) -> dict:
        ...
        if watch_type == "forward":
            ...
            rule = make_forward_watch_rule(
                ...
                resolve_deep_link,
                archive_by_author,
                archive_title_source,
            )
            same_target_exists = any(
                parse_forward_watch_rule(existing).get("source_link") == source_link
                and parse_forward_watch_rule(existing).get("target_link") == target_link
                for existing in self.listen_forward_chat
            )
            same_persisted_exists = any(
                watch.get("type") == "forward"
                and watch.get("source_link") == source_link
                and watch.get("target_link") == target_link
                for watch in self.persisted_watches()
            )
    ...
    def delete_watch(self, watch_id: str) -> bool:
        ...
        if watch_type == "download":
            ...
            if not handler:
                pending_deleted = (
                    self.web_pending_watches.pop(watch_id, None) is not None
                )
                store_deleted = (
                    store.delete_live_transfer_watch(watch_id) if store else False
                )
                if store:
            ...
            client = (
                ...
                or self._user
                or (self._app.client if self._app else None)
            )
            if client:
            ...
            stale_keys = [
                k for k, v in self._download_chat_watch_id.items() if v == watch_id
            ]
            for k in stale_keys:
    ...
    def update_watch(self, watch_id: str, payload: dict) -> dict:
        ...
        if "comment_delay_minutes" in payload:
            ...
            new_comment_delay_minutes = (existing or {}).get("comment_delay_minutes")
        media_types = payload.get("media_types")
        ...
        if new_rule == value:
            existing = (
                self._transfer_store.get_live_transfer_watch(watch_id)
    ...
    def export_forward_watches(self) -> dict:
        ...
        watches = [
            watch for watch in self.list_watches() if watch.get("type") == "forward"
        ]
    ...
    def list_watch_events(
        ...
        return {
            ...
            "status_counts": store.count_live_watch_events_by_status(
                watch_id, today_only=today_only, tz_offset_minutes=tz_offset_minutes
            ),
