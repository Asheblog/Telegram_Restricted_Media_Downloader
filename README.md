...
部分频道帖只有封面/`t.me/<bot>?start=...` 按钮，正片在资源 bot 私聊里。用法：

1. **系统设置 → 深链取片**：填写资源 bot 白名单（每行一个用户名，如 `123456`），可改超时秒数（默认 60）。
2. 新建转存或监听转发时勾选 **深链取片**（白名单为空时无法勾选创建）。
3. 命中白名单深链时，会用已登录账号向该 bot 取 `video`/`document`/`animation`，再转存到目标；失败不会用频道预览顶替。
