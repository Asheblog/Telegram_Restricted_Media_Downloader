...
10. `/forward`命令使用教程：
	...
	> 当受限内容采用"下载后上传"时，下载后上传队列默认最多保留`3`个本地待上传任务，可通过`[帮助页面]`->`[设置]`->`[上传设置]`在`1~5`之间调整，避免大量视频先全部下载到本地。
	> 自版本`≥v1.6.9`起：  
	> `/forward`将支持过滤转发类型。  
...
13. `/listen_forward`命令使用教程：
	...
	> 下载后上传队列默认最多保留`3`个本地待上传任务，可通过`[帮助页面]`->`[设置]`->`[上传设置]`在`1~5`之间调整。
...
upload:
  delete: false # 控制/listen_forward与/forward命令遇到受限内容时,下载上传完成后是否删除已上传完成的本地文件。
  download_upload: true # 控制/listen_forward与/forward命令遇到受限内容时,是否下载后上传到指定的转发频道。
  pending_limit: 3 # 控制/listen_forward与/forward命令下载后上传时,最多保留几个本地待上传任务。支持的参数:1~5。
```
...
7. `-w`、`--web`参数用法：
   ...
   | _2.WebUI 是命令中心，覆盖 `/help`、`/download`、`/table`、`/forward`、`/exit`、`/listen_download`、`/listen_forward`、`/listen_info`、`/upload`、`/upload_r`、`/download_chat`。_ |
   | _3.浏览器无法接收 Telegram 原生转发消息；WebUI 使用“消息链接导入”作为等价入口，可粘贴单条或多条消息链接来创建下载任务。_ |
   | _4.遇到受限内容时，系统会下载媒体并重新发送到目标会话；PikPak 模式默认以文档形式发送。_                          |
   | _5.Web Command Task 状态会保存到 `temp_directory` 下的 SQLite 数据库，程序重启后会继续处理待处理任务。_                          |
   | _6.可在此参数后指定一个`0`~`65535`范围内的**端口号**，若不指定，将**随机分配**端口。_       |
   | _7.默认仅监听 `127.0.0.1`；Docker/远程访问需要显式设置环境变量 `TRMD_WEB_HOST=0.0.0.0`。_                                           |
   | _8.远程监听时必须同时设置 `TRMD_WEB_USERNAME` 与 `TRMD_WEB_PASSWORD`，浏览器会弹出基础认证登录框。_                                           |
   | _9.这是一次直接替换：旧 ttyd Web 入口和旧单一转存页面无迁移，直接替换为 WebUI 命令中心。_                                           |
