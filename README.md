...
- PikPak 云盘按来源频道归档默认开启，默认使用 rclone remote `pikpak`，把文件移动到 `Telegram/来源频道`。请先在运行环境配置 rclone PikPak remote；TRMD 不保存 PikPak 账号密钥，只调用本机 rclone 将 PikPak bot 入库后的文件移动到 `root_directory/来源频道`。
