...
**Author Archive Reorganize** — WebUI 工具：按 Source Channel Folder 扫描 PikPak 归档中的主贴目录，回查 Telegram 主贴解析 Post Author（署名 → 标签回连已知作者 → 未识别），生成迁移一览（汇总可点开分页明细）；「全部迁移」一键执行高置信 + 待确认，未识别默认不搬。「仅解析未识别」保留上次已识别结果，只回查未识别主贴（含相册兄弟 hashtag）。与实时链路的 `archive_by_author` 解耦，仅用于显式历史整理。
