...
**Deferred Discussion Reply Capture** — 监听转发在开启 Discussion Reply Inclusion 时，主贴立即转发后，将讨论区抓取推迟到配置的到期时刻再执行一次的持久化任务。执行中可取消；失败或已取消可手动重试；超时未结束视为失败。
_Avoid_: Comment delay job, deferred comment poll, comment scrape loop
