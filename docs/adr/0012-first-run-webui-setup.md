...
`--web` 模式下，把原先依赖 TTY/`docker exec` 的初始化（`api_id`/`api_hash`、Telegram 登录、rclone PikPak）收成 WebUI 内多步向导；站内登录仍先于向导（ADR-0003）。已具备有效 API 凭证与 Telegram Session 的升级环境按真实状态判定就绪，不弹全屏向导。**新装引导必须配通 rclone**（探测成功后开启归档）：下载回退不再经 `pikpak_bot`，改为 rclone 直传到 `My Telegram` 再复用归档。CLI 无 `--web` 时仍走原 stdin `config_guide`。
