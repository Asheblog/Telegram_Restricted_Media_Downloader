...
`--web` 模式下，把原先依赖 TTY/`docker exec` 的初始化（`api_id`/`api_hash`、Telegram 登录、rclone PikPak）收成 WebUI 内多步向导；站内登录仍先于向导（ADR-0003）。已具备有效 API 凭证与 Telegram Session 的升级环境按真实状态判定就绪，不弹全屏向导。rclone 可跳过：跳过或失败时保持归档关闭；探测成功后再开启。CLI 无 `--web` 时仍走原 stdin `config_guide`。

**Considered Options**

- 仅补 rclone、不改启动序：空配置仍卡在 `config_guide` stdin，WebUI 起不来。
- 强制配通 rclone 才能进主界面：卡住只想先转存、暂不归档的用户。
