...
评论区深链：同时开启 Discussion Reply Inclusion 时，讨论回复（含延迟抓取）扫描白名单深链并 resolve；仅含白名单深链的纯文本评论可进入该路径（绕过 `forward_type.text`）。监听转发与 Web 转存共用此规则。主贴无深链时不会在主贴瞬间主动嗅探评论（依赖「包含评论区」+ 延迟抓取，避免竞态）。
