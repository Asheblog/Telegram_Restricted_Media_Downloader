...
class WatchInlineDownloadRecordsCase(unittest.TestCase):
    ...
    def test_attach_download_counts_to_watches(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            rule = make_forward_watch_rule(
                "https://t.me/ctuxas", "https://t.me/pikpak_bot"
            )
            ...
            store.update_task(
                completed_id, status=TransferStatus.SUCCESS, finished=True
            )
            store.update_task(
                legacy_completed_id, status=TransferStatus.SUCCESS, finished=True
            )
    ...
    def test_watch_download_tasks_expose_display_file_name_and_active_progress(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            store.update_task(
                active_id, status=TransferStatus.RUNNING, total_items=1, started=True
            )
            store.update_task(
                done_id, status=TransferStatus.SUCCESS, total_items=1, finished=True
            )
            store.add_item(
                task_id=active_id,
