...
class FakeWebUiOperations:
    ...
    def import_forward_watches(self, payload):
        ...
        return import_forward_watch_entries(
            entries, self.create_watch, parse_errors=parse_errors
        )
...
class TransferStoreWebUiCase(unittest.TestCase):
    ...
    def test_log_cleanup_removes_rotated_files_older_than_three_days(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            removed = trmd_module.cleanup_old_log_files(
                log_path=log_path, retention_days=3, now=now
            )
    ...
    def test_transfer_store_purges_old_live_watch_and_cleanup_log_records(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            store.upsert_live_transfer_watch(
                watch_id, "download", "https://t.me/source"
            )
            ...
            store.add_live_watch_event(
                watch_id, "123", 1, None, None, "success", "old event"
            )
            with store.connect(run_maintenance=False) as conn:
                conn.execute(
                    "UPDATE live_watch_events SET created_at = ?", (old_watch_cutoff,)
                )
            ...
            with store.connect(run_maintenance=False) as conn:
                conn.execute(
                    "UPDATE cleanup_log SET created_at = ?", (old_cleanup_cutoff,)
                )
    ...
    def test_transfer_store_purges_old_system_logs(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            with store.connect(run_maintenance=False) as conn:
                conn.execute("UPDATE system_logs SET created_at = ?", (old_cutoff,))
    ...
    def test_list_system_logs_supports_filters_and_pagination(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            store.add_system_log(
                "filter", "filter_reject", "filter log", level="warning"
            )
            store.add_system_log(
                "archive", "archive_success", "archive log", level="info"
            )
    ...
    def test_transfer_store_read_paths_use_covering_indexes(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            self.assertIn(
                "IDX_DOWNLOAD_RECORDS_UPDATED_ORDER", flattened["record_list"]
            )
    ...
    def test_download_success_record_is_reused_only_when_file_is_valid(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            self.assertIsNone(
                store.get_download_success_record("-100123", 42, expected_size=5)
            )

    def test_default_download_concurrency_and_pikpak_size_limit_are_system_defaults(
        self,
    ):
    ...
    def test_user_config_normalizes_runtime_numeric_settings(self):
        ...
        config = UserConfig.normalize_runtime_numbers(
            {
                "max_tasks": {"download": "2", "upload": "0"},
                "max_retries": {"download": "7", "upload": None},
            }
        )
    ...
    def test_update_web_settings_keeps_runtime_task_limits_numeric_from_string_config(
        self,
    ):
        from module.downloader import TelegramRestrictedMediaDownloader
        ...
        downloader = TelegramRestrictedMediaDownloader.__new__(
        ...
        downloader.gc = SimpleNamespace(
            config={"notice": True}, save_config=save_global_config
        )
        downloader.download_upload_window = SimpleNamespace(
            notify_limit_changed=lambda: None
        )
    ...
    def test_global_target_profile_archive_config_is_completed_recursively(self):
        ...
        config = {
            ...
            "target_profiles": {
                "pikpak": {"max_file_size": 1024, "archive": {"enable": True}}
            },
            "forward_type": {
                "video": True,
                "photo": True,
    ...
    def test_transfer_task_persists_discussion_reply_inclusion(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            task_id = store.create_task(
                "https://t.me/source", "https://t.me/pikpak_bot", include_comment=True
            )
    ...
    def test_task_progress_counts_delete_and_download_records_are_public_behaviors(
        self,
    ):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            store = TransferStore(directory=directory)
            task_id = store.create_task(
                "https://t.me/source/1", "https://t.me/pikpak_bot"
            )
            ...
            store.update_item_progress(
                item_id, phase="downloading", download_current=4, download_total=10
            )
            ...
            store.refresh_task_counts(
                task_id, expected_total=1, assignment_completed=True
            )
            task = store.get_task(task_id)
    ...
    def test_webui_view_model_is_the_public_task_contract(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            store.update_item_progress(
                failed_item, phase="failed", upload_current=3, upload_total=10
            )
            store.update_item(skipped_item, phase="skipped")
            store.refresh_task_counts(
                task_id, expected_total=4, assignment_completed=False
            )

            model = WebUiViewModel(store)
            task_list = model.task_list()["tasks"]
            detail = model.task_detail(task_id, item_limit=10, event_limit=10)
            failed_detail = model.task_detail(
                task_id, item_limit=10, item_status=TransferStatus.FAILURE
            )
            ...
            self.assertEqual(
                TransferStatus.FAILURE, failed_detail["items"][0]["status"]
            )
    ...
    def test_webui_settings_model_is_the_public_settings_contract(self):
        settings = {
            ...
            "global": {
                ...
                "message_filter": {"media_types": {"video": True, "text": False}},
            },
        }
        schema = {
            "download_type": ["video", "photo", "document"],
            "forward_type": ["video", "photo"],
            "message_filter": {"media_types": ["video", "text"]},
        }
        ...
        self.assertEqual(
            ["video", "document"], model["selections"]["user_download_type"]
        )
        self.assertEqual(
            {"video": True, "photo": False}, model["selections"]["forward_type"]
        )
        self.assertEqual(
            {"video": True, "text": False},
            model["selections"]["message_filter_media_types"],
        )

    def test_webui_exposes_delete_settings_and_download_records_without_secret_leaks(
        self,
    ):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            media_path = os.path.join(directory, "media.bin")
    ...
    def test_webui_download_records_support_pagination_and_clear(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                conn.request(
                    "GET", "/api/download-records?limit=2&offset=2", headers=headers
                )
                response = conn.getresponse()
                body = json.loads(response.read().decode("utf-8"))
                self.assertEqual(200, response.status)
    ...
    def test_webui_task_api_detects_transfer_range_when_chat_link_has_no_ids(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                self.assertEqual(
                    ["https://t.me/source"], operations.detected_transfer_ranges
                )
            finally:
                server.stop()
    ...
    def test_webui_task_api_keeps_message_link_without_auto_range(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            operations.transfer_range = {"start_id": 1, "end_id": 9}
            server = WebUiServer(
                store=store, operations=operations, username="admin", password="pass"
            )
            server.start(open_browser=False)
            headers = self._authenticated_headers(
                server, content_type="application/json"
    ...
    def test_webui_task_retry_failed_resets_failed_items_and_resubmits_task(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            store = TransferStore(directory=directory)
            task_id = store.create_task(
                "https://t.me/source", "https://t.me/pikpak_bot", start_id=1, end_id=2
            )
            ...
            store.refresh_task_counts(
                task_id, expected_total=2, assignment_completed=True
            )
            submitted = []
            server = WebUiServer(
                store=store,
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            downloader.pikpak_manager = PikpakIntegrationManager(
                ...
                refresh_counts=lambda tid: (
                    s.refresh_task_counts(tid)
                    if (s := downloader.__dict__.get("transfer_store"))
                    else None
                ),
            )
            downloader.submit_web_task = lambda submitted_task_id: submitted.append(
                submitted_task_id
            )
            ...
            self.assertEqual(
                "73962 - 作者_ #示例社区 #示例标签.mp4", archive_calls[0]["file_name"]
            )
            ...
            self.assertTrue(
                any("recovered before retry" in event["message"] for event in events)
            )

    def test_downloader_retry_failed_recovers_pikpak_archive_failure_before_resubmitting(
        self,
    ):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
                ...
                def archive_file(self, **kwargs):
                    ...
                    if kwargs.get("file_name") == "done.mp4":
                        return SimpleNamespace(
                            ok=True,
                            status="success",
                            archive_path="Telegram/chengdudiyi8/done.mp4",
                        )
                    return SimpleNamespace(
                        ok=False, status="not_found", message="not indexed yet"
                    )
    ...
    def test_webui_task_pause_and_resume_api_use_operations(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                self.assertEqual(
                    TransferStatus.PAUSED, store.get_task(task_id)["status"]
                )
                ...
                self.assertEqual(
                    TransferStatus.PENDING, store.get_task(task_id)["status"]
                )
                self.assertEqual(
                    [("pause", task_id), ("resume", task_id)], operations.calls
                )
            finally:
                server.stop()
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                conn.request(
                    ...
                    "/api/watches",
                    body=json.dumps(
                        {"type": "download", "source_links": ["https://t.me/source"]}
                    ),
                    headers={**headers, "Content-Type": "application/json"},
                )
                response = conn.getresponse()
    ...
    def test_webui_exports_and_imports_forward_watches(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                self.assertEqual(200, response.status)
                self.assertIn(
                    'attachment; filename="forward-watches-',
    ...
    def test_webui_exposes_upload_channel_download_and_forward_submission(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                self.assertEqual(
                    ["video", "photo"],
                    operations.created_channel_downloads[0]["download_type"],
                )
            finally:
                server.stop()
    ...
    def test_webui_forward_watch_accepts_discussion_reply_inclusion(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                self.assertTrue(
                    operations.watches[
                        "forward:https://t.me/source->https://t.me/target"
                    ]["include_comment"]
                )
            finally:
                server.stop()
    ...
    def test_webui_forward_watch_accepts_comment_delay_override(self):
        ...
        try:
            ...
            self.assertEqual(
                120,
                operations.watches["forward:https://t.me/slow->https://t.me/target"][
    ...
    def test_webui_transfer_tries_native_forward_before_restricted_fallback_download(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            downloader.wait_for_pikpak_ingest_confirmation = AsyncMock(
                return_value=True
            )
            ...
            self.assertEqual(
                [1, 2], [call["message_id"] for call in downloader.forward_calls]
            )
            self.assertTrue(
                all(call["ignore_type_filter"] for call in downloader.forward_calls)
            )
    ...
    def test_webui_transfer_includes_discussion_replies_when_enabled(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            with (
                patch("module.downloader.parse_link", side_effect=fake_parse_link),
                patch.object(
                    downloader, "wait_between_transfer_messages", new=AsyncMock()
                ),
            ):
                asyncio.run(downloader.process_web_transfer_task(task_id))

            self.assertEqual(
                [1, 10], [call["message_id"] for call in downloader.forward_calls]
            )
            self.assertEqual(
                "discussion-chat", downloader.forward_calls[1]["origin_chat_id"]
            )
            task = store.get_task(task_id)
            self.assertEqual(2, task["total_items"])
            self.assertEqual(2, task["completed_items"])
    ...
    def test_webui_discussion_reply_without_link_fallback_uses_message_object(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            downloader.app = SimpleNamespace(client=FakeClient())
            downloader.gc = SimpleNamespace(
                download_upload=True, upload_delete=False, forward_type={"video": True}
            )
            downloader.download_calls = []
            ...
            reply_count, fallback_count = asyncio.run(
                downloader.transfer_web_discussion_replies_to_target(
                    task=store.get_task(task_id),
                    source_chat_id="source-chat",
                    source_message_id=1,
    ...
    def test_direct_forward_updates_task_progress_before_assignment_completes(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            store.refresh_task_counts(
                task_id, expected_total=2, assignment_completed=False
            )
            ...
            asyncio.run(
                downloader.transfer_message_to_web_target(
                    task=task,
    ...
    def test_direct_pikpak_forward_archive_schedules_instead_of_blocking(self):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            store = TransferStore(directory=directory)
            ...
            store.refresh_task_counts(
                task_id, expected_total=1, assignment_completed=False
            )
            task = store.get_task(task_id)
            scheduled = []
            ...
            class FakeArchiveClient:
                ...
                def archive_file(self, **kwargs):
                    raise AssertionError(
                        "direct forward must not archive synchronously"
                    )
            ...
            self.assertTrue(str(scheduled[0]["source_folder"]).startswith("ctuxas"))
            self.assertEqual("video.mp4", scheduled[0]["file_name"])
            self.assertTrue(scheduled[0]["match_original_name"])
            self.assertEqual(0, scheduled[0]["delay_seconds"])
            item = store.list_items(task_id)[0]
            self.assertEqual(TransferStatus.SUCCESS, item["status"])
            self.assertEqual("forwarded", item["phase"])
            ...
            events = store.list_events(task_id)
            self.assertTrue(
                any("archive scheduled" in event["message"] for event in events)
            )
    ...
    def test_direct_pikpak_forward_archives_with_message_title_filename(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            self.assertEqual(1, len(scheduled))
            self.assertIn("chengdudiyi8/73962", scheduled[0]["source_folder"])
            self.assertTrue(str(scheduled[0]["file_name"]).endswith(".mp4"))
            self.assertFalse(scheduled[0]["match_original_name"])
            item = store.list_items(task_id)[0]
            self.assertEqual(TransferStatus.SUCCESS, item["status"])
            self.assertEqual("pending", item["archive_status"])
            self.assertTrue(str(item["file_name"] or "").endswith(".mp4"))
    ...
    def test_webui_transfer_skips_empty_source_message_without_forwarding(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            self.assertIn(
                "Telegram API returned an empty source message",
                items[0]["error_message"],
            )
            task = store.get_task(task_id)
            self.assertEqual(1, task["completed_items"])
    ...
    def test_direct_pikpak_forward_timeout_recovers_when_archive_finds_ingested_file(
        self,
    ):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            class FakeArchiveClient:
                def archive_file(self, **kwargs):
                    ...
                    return SimpleNamespace(
                        ok=True,
                        status="success",
                        archive_path="Telegram/chengdudiyi8/video.mp4",
                    )
            ...
            self.assertEqual(1, len(archive_calls))
            self.assertTrue(
                str(archive_calls[0]["source_folder"]).startswith("chengdudiyi8")
            )
            ...
            events = store.list_events(task_id)
            self.assertTrue(
                any("recovered by archive" in event["message"] for event in events)
            )
    ...
    def test_webui_pikpak_confirmation_failure_continues_range_assignment(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            events = store.list_events(task_id)
            self.assertFalse(
                any("Transfer task failed" in event["message"] for event in events)
            )
    ...
    def test_direct_pikpak_forward_with_ingest_confirmation_records_success(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            class FakeArchiveClient:
                ...
                def archive_file(self, **kwargs):
                    ...
                    return SimpleNamespace(
                        ok=True,
                        status="success",
                        archive_path="Telegram/source/video.mp4",
                    )
            ...
            self.assertEqual(1, len(items))
            self.assertEqual(TransferStatus.SUCCESS, items[0]["status"])
            self.assertEqual("pending", items[0]["archive_status"])
            self.assertEqual(1, store.get_task(task_id)["completed_items"])
            self.assertEqual(1, len(scheduled))
    ...
    def test_pikpak_ingest_confirmation_requires_forwarded_message_identity(self):
        ...
        self.assertFalse(
            asyncio.run(
                downloader.wait_for_pikpak_ingest_confirmation(
                    target_chat_id="target-chat",
    ...
    def test_pikpak_ingest_confirmation_default_timeout_is_short(self):
        ...
        signature = inspect.signature(
            TelegramRestrictedMediaDownloader.wait_for_pikpak_ingest_confirmation
        )
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            class FakeArchiveClient:
                ...
                def archive_file(self, **kwargs):
                    archive_calls.append(kwargs)
                    return SimpleNamespace(
                        ok=False, status="missing_metadata", message="metadata missing"
                    )
            ...
            downloader.archive_pikpak_item = (
                downloader.pikpak_manager.archive_pikpak_item
            )
            downloader.refresh_transfer_task_counts = lambda tid: (
                store.refresh_task_counts(tid)
            )
            downloader.runtime_message_filter = lambda override=None: (
                build_runtime_message_filter(
            ...
            self.assertFalse(
                any(
                    "PikPak archive missing_metadata" in event["message"]
                    for event in events
                )
            )
    ...
    def test_pikpak_upload_status_archives_without_transfer_store(self):
        ...
        class FakeArchiveClient:
            def archive_file(self, **kwargs):
                archive_calls.append(kwargs)
                return SimpleNamespace(
                    ok=True, status="success", archive_path="Telegram/ctuxas/video.mp4"
                )
    ...
    def test_pikpak_upload_archive_failure_keeps_upload_success_and_records_archive_error(
        self,
    ):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
    ...
    def test_downloader_retry_failed_recovers_pikpak_upload_archive_failure_before_resubmitting(
        self,
    ):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
    ...
    def test_common_download_upload_meta_enables_pikpak_archive_callbacks_for_listen_forward(
        self,
    ):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
        ...
        self.assertEqual(
            "on_transfer_upload_status", meta["status_callback"].__func__.__name__
        )
        self.assertEqual("on_transfer_file_ready", meta["on_file_ready"].__name__)
        self.assertEqual(
            "on_transfer_file_ready", meta["on_file_ready"].__func__.__name__
        )
    ...
    def test_pikpak_transfer_over_target_limit_skips_before_forward_or_download(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            self.assertTrue(
                any(
                    event["level"] == "warning" and "PikPak" in event["message"]
                    for event in events
                )
            )
            task = store.get_task(task_id)
    ...
    def test_webui_transfer_resumes_running_range_without_repeating_completed_items(
        self,
    ):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            self.assertEqual(
                [2], [call["message_id"] for call in downloader.forward_calls]
            )
            task = store.get_task(task_id)
            self.assertEqual(2, task["total_items"])
            self.assertEqual(2, task["completed_items"])
    ...
    def test_webui_transfer_skips_missing_range_messages_and_continues(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            self.assertEqual(
                [1, 3], [call["message_id"] for call in downloader.forward_calls]
            )
            items = store.list_items(task_id)
            ...
            self.assertTrue(
                any(
                    event["level"] == "warning" and "2" in event["message"]
                    for event in events
                )
            )
            task = store.get_task(task_id)
    ...
    def test_downloader_detects_transfer_range_from_accessible_chat_history(self):
        ...
        class FakeClient:
            def __init__(self):
                self.pages = [[SimpleNamespace(id=99)], [SimpleNamespace(id=3)], []]
                self.calls = []
    ...
    def test_downloader_detects_transfer_range_uses_count_offset_fast_path(self):
        ...
        class FakeClient:
            ...
            async def get_chat_history(
                self, chat_id, limit=0, offset=0, offset_id=0, **_kwargs
            ):
                self.calls.append(
                    {
                        "chat_id": chat_id,
    ...
    def test_downloader_detects_transfer_range_falls_back_when_fast_tail_matches_newest(
        self,
    ):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
    ...
    def test_forward_waits_and_retries_copy_message_flood_wait(self):
        ...
        async def run_case():
            with (
                patch("module.downloader.asyncio.sleep") as sleep_mock,
    ...
    def test_forward_logs_pikpak_archive_failure_without_transfer_store(self):
        ...
        self.assertTrue(
            any("PikPak archive not_found" in message for message in logs.output)
        )
    ...
    def test_forward_media_group_archives_each_member_to_pikpak(self):
        ...
        self.assertEqual(
            "2 - 共同标题.jpg",
            PikpakIntegrationManager.get_message_media_archive_filename(photo_message),
        )
    ...
    def test_webui_start_requeues_running_tasks_after_container_restart(self):
        ...
        fake_web_ui = SimpleNamespace(
            start=lambda open_browser: None, url="http://127.0.0.1:8080"
        )
    ...
    def test_deleting_running_web_task_cancels_and_schedules_next_task(self):
        ...
        async def run_case():
            ...
            with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
                ...
                next_task_id = store.create_task(
                    "https://t.me/source/2", "https://t.me/pikpak_bot"
                )
                downloader.transfer_store = store
                ...
                if downloader.web_running_task:
                    ...
                    await asyncio.gather(
                        downloader.web_running_task, return_exceptions=True
                    )
    ...
    def test_bot_forward_and_listen_forward_parse_discussion_reply_flag(self):
        ...
        message.text = (
            "/listen_forward https://t.me/source https://t.me/target --include-comment"
        )
        listen_meta = asyncio.run(Bot.on_listen(bot, client, message))
        self.assertTrue(listen_meta["include_comment"])
        self.assertEqual(
            ["https://t.me/source", "https://t.me/target"], listen_meta["links"]
        )
    ...
    def test_listen_forward_includes_discussion_replies_when_enabled(self):
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            store = TransferStore(directory=directory)
            downloader.app = SimpleNamespace(
                client=FakeClient(), temp_directory=directory
            )
            downloader.transfer_store = store
            downloader.gc = SimpleNamespace(
                forward_type={"video": True, "photo": False, "text": False},
            ...
            with patch(
                "module.transfer.live_transfer.parse_link", side_effect=fake_parse_link
            ):
                asyncio.run(downloader.listen_forward(object(), FakeMessage()))

            self.assertEqual(
                [5, 15], [call["message_id"] for call in downloader.forward_calls]
            )
            self.assertEqual(
                "discussion-chat", downloader.forward_calls[1]["origin_chat_id"]
            )
    ...
    def test_listen_forward_downloads_discussion_reply_when_direct_copy_fails(self):
        ...
        downloader.app = SimpleNamespace(client=FakeClient())
        downloader.gc = SimpleNamespace(
            download_upload=True, upload_delete=False, message_filter={"enabled": False}
        )
        downloader.download_calls = []
        ...
        count = asyncio.run(
            downloader.forward_discussion_replies(
    ...
    def test_webui_accepts_non_recursive_directory_upload_for_upload_command_parity(
        self,
    ):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                self.assertTrue(body["accepted"])
                self.assertEqual(
                    os.path.abspath(directory), operations.created_uploads[0]["path"]
                )
    ...
    def test_webui_live_watch_delete_uses_client_that_registered_handler(self):
        ...
        async def exercise_watch_lifecycle():
            await downloader.apply_web_watch(
                {"watch_type": "download", "source_link": "https://t.me/source"}
            )
    ...
    def test_webui_live_watch_persists_and_restores_after_restart(self):
        ...
        def build_downloader(store):
            ...
            downloader.watch_manager = LiveWatchManager(
                ...
                web_pending_watches=downloader.web_pending_watches,
                web_watch_handler_clients=downloader.web_watch_handler_clients,
                transfer_store_getter=lambda: getattr(
                    downloader, "transfer_store", None
                ),
                operation_submitter=downloader.submit_web_operation,
                user_getter=lambda: getattr(downloader, "user", None),
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            try:
                ...
                self.assertEqual(
                    TransferStatus.RUNNING, restored.list_watches()[0]["status"]
                )
                ...
                self.assertEqual(
                    restored.user.added_handlers, restored.user.removed_handlers
                )
            finally:
                original_loop.close()
                restored_loop.close()

    def test_webui_live_watch_delete_defaults_to_user_client_for_existing_bot_watches(
        self,
    ):
        TelegramRestrictedMediaDownloader = import_downloader_class()
        downloader = object.__new__(TelegramRestrictedMediaDownloader)
    ...
    def test_live_watch_manager_reports_total_and_today_event_counts(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            old_at = (
                datetime.datetime.fromisoformat(today_start)
            ...
            today_events = manager.list_watch_events(
                watch_id, limit=50, offset=0, today_only=True
            )
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            server = WebUiServer(
                ...
                operations=SimpleNamespace(
                    list_watch_events=lambda *args, **kwargs: manager.list_watch_events(
                        *args, **kwargs
                    )
                ),
                username="admin",
    ...
    def test_live_watch_today_count_respects_client_timezone_offset(self):
        ...
        with patch("module.transfer_store.datetime.datetime") as dt_mock:
            ...
            with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
                ...
                self.assertEqual(
                    0,
                    store.get_live_watch_event_count(
                        watch_id, today_only=True, tz_offset_minutes=0
                    ),
                )
                self.assertEqual(
                    1,
                    store.get_live_watch_event_count(
                        watch_id, today_only=True, tz_offset_minutes=-480
                    ),
                )
    ...
    def test_webui_item_model_exposes_upload_and_download_speed(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            detail = WebUiViewModel(store).task_detail(
                task_id, item_status=TransferStatus.RUNNING
            )
            item = detail["items"][0]
    ...
    def test_range_progress_counts_completed_ids_while_earlier_async_items_still_run(
        self,
    ):
    ...
    def test_is_range_message_complete_requires_all_comment_items_terminal(self):
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
            ...
            self.assertEqual(
                1, len(store.list_resumable_items_for_range_message(task_id, 10))
            )
        ...
        async def run_case():
            with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as directory:
                ...
                with (
                    patch("module.downloader.parse_link", side_effect=fake_parse_link),
                    patch(
                        "module.transfer.runner.iter_discussion_reply_messages",
                        new=fake_discussion_replies,
                    ),
                    patch.object(
                        downloader, "wait_between_transfer_messages", new=AsyncMock()
                    ),
